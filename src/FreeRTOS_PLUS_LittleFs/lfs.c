...
#define cpy_dir(dst_ptr, src_ptr)                          \
    do {                                          \
        memcpy(dst_ptr, src_ptr, sizeof(lfs_mdir_t)); \
    } while (0)
...
static int lfs_bd_read(lfs_t             *lfs,
    ...
    while (size > 0) {
        ...
        rcache->size =
            lfs_min(lfs_min(lfs_alignup(off + hint, lfs->cfg->read_size), lfs->cfg->block_size) -
                        rcache->off,
                    lfs->cfg->cache_size);
        int err =
            lfs->cfg->read(lfs->cfg, rcache->block, rcache->off, rcache->buffer, rcache->size);
        LFS_ASSERT(err <= 0);
        if (err) {
            return err;
        }
...
static int lfs_bd_flush(lfs_t *lfs, lfs_cache_t *pcache, lfs_cache_t *rcache, bool validate) {
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE) {
        ...
        if (validate) {
            ...
            int res = lfs_bd_cmp(lfs, NULL, rcache, diff, pcache->block, pcache->off,
                                 pcache->buffer, diff);
            if (res < 0) {
                return res;
            }
...
static inline lfs_size_t lfs_path_namelen(const char *path) { return strcspn(path, "/"); }
...
static inline bool lfs_path_isdir(const char *path) { return path[lfs_path_namelen(path)] != '\0'; }
...
static inline int lfs_pair_cmp(const lfs_block_t paira[2], const lfs_block_t pairb[2]) {
    return !(paira[0] == pairb[0] || paira[1] == pairb[1] || paira[0] == pairb[1] ||
             paira[1] == pairb[0]);
}
...
static inline bool lfs_tag_isvalid(lfs_tag_t tag) { return !(tag & 0x80000000); }

static inline bool lfs_tag_isdelete(lfs_tag_t tag) { return ((int32_t)(tag << 22) >> 22) == -1; }

static inline uint16_t lfs_tag_type1(lfs_tag_t tag) { return (tag & 0x70000000) >> 20; }

static inline uint16_t lfs_tag_type2(lfs_tag_t tag) { return (tag & 0x78000000) >> 20; }

static inline uint16_t lfs_tag_type3(lfs_tag_t tag) { return (tag & 0x7ff00000) >> 20; }

static inline uint8_t lfs_tag_chunk(lfs_tag_t tag) { return (tag & 0x0ff00000) >> 20; }

static inline int8_t lfs_tag_splice(lfs_tag_t tag) { return (int8_t)lfs_tag_chunk(tag); }

static inline uint16_t lfs_tag_id(lfs_tag_t tag) { return (tag & 0x000ffc00) >> 10; }

static inline lfs_size_t lfs_tag_size(lfs_tag_t tag) { return tag & 0x000003ff; }
...
#ifndef LFS_READONLY
static inline bool lfs_gstate_hasorphans(const lfs_gstate_t *a) { return lfs_tag_size(a->tag); }

static inline uint8_t lfs_gstate_getorphans(const lfs_gstate_t *a) {
    return lfs_tag_size(a->tag) & 0x1ff;
}

static inline bool lfs_gstate_hasmove(const lfs_gstate_t *a) { return lfs_tag_type1(a->tag); }
#endif
...
#ifndef LFS_READONLY
static int
lfs_dir_commit(lfs_t *lfs, lfs_mdir_t *dir, const struct lfs_mattr *attrs, int attrcount);
static int lfs_dir_compact(lfs_t                  *lfs,
                           ...
                           uint16_t                end);
static lfs_ssize_t
lfs_file_flushedwrite(lfs_t *lfs, lfs_file_t *file, const void *buffer, lfs_size_t size);
static lfs_ssize_t
           lfs_file_write_(lfs_t *lfs, lfs_file_t *file, const void *buffer, lfs_size_t size);
...
static lfs_ssize_t
lfs_file_flushedread(lfs_t *lfs, lfs_file_t *file, void *buffer, lfs_size_t size);
...
static void lfs_alloc_ckpoint(lfs_t *lfs) { lfs->lookahead.ckpoint = lfs->block_count; }
...
static lfs_stag_t lfs_dir_getslice(lfs_t            *lfs,
    ...
    while (off >= sizeof(lfs_tag_t) + lfs_tag_dsize(ntag)) {
        ...
        lfs_tag_t tag = ntag;
        int       err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(ntag), dir->pair[0], off, &ntag,
                                    sizeof(ntag));
        LFS_ASSERT(err <= 0);
        if (err) {
            return err;
        ...
        if (lfs_tag_id(gmask) != 0 && lfs_tag_type1(tag) == LFS_TYPE_SPLICE &&
            ...
            if (tag ==
                (LFS_MKTAG(LFS_TYPE_CREATE, 0, 0) | (LFS_MKTAG(0, 0x3ff, 0) & (gtag - gdiff)))) {
        ...
        if ((gmask & tag) == (gmask & (gtag - gdiff))) {
            ...
            err = lfs_bd_read(lfs, NULL, &lfs->rcache, diff, dir->pair[0], off + sizeof(tag) + goff,
                              gbuffer, diff);
            LFS_ASSERT(err <= 0);
            if (err) {
                return err;
...
static lfs_stag_t
lfs_dir_get(lfs_t *lfs, const lfs_mdir_t *dir, lfs_tag_t gmask, lfs_tag_t gtag, void *buffer) {
...
static int lfs_dir_getread(lfs_t             *lfs,
    ...
    while (size > 0) {
        ...
        int err =
            lfs_dir_getslice(lfs, dir, gmask, gtag, rcache->off, rcache->buffer, rcache->size);
        if (err < 0) {
            return err;
        }
    }
...
static int lfs_dir_traverse_filter(void *p, lfs_tag_t tag, const void *buffer) {
    ...
    uint32_t mask =
        (tag & LFS_MKTAG(0x100, 0, 0)) ? LFS_MKTAG(0x7ff, 0x3ff, 0) : LFS_MKTAG(0x700, 0x3ff, 0);
    ...
    while (true) {
        {
            if (off + lfs_tag_dsize(ptag) < dir->off) {
                ...
                int err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(tag), dir->pair[0], off, &tag,
                                      sizeof(tag));
                if (err) {
                    return err;
                }
    ...
    popped:
        ...
        if (lfs_tag_type3(tag) == LFS_FROM_NOOP) {
            ...
            for (unsigned i = 0; i < lfs_tag_size(tag); i++) {
                ...
                res =
                    cb(data,
                       LFS_MKTAG(LFS_TYPE_USERATTR + a[i].type, lfs_tag_id(tag) + diff, a[i].size),
                       a[i].buffer);
                if (res < 0) {
                    return res;
                }
...
static lfs_stag_t lfs_dir_fetchmatch(lfs_t            *lfs,
    ...
    int      r = 0;
    for (int i = 0; i < 2; i++) {
        int err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(revs[i]), pair[i], 0, &revs[i],
                              sizeof(revs[i]));
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (true) {
            ...
            lfs_tag_t tag;
            off += lfs_tag_dsize(ptag);
            int err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->block_size, dir->pair[0], off,
                                  &tag, sizeof(tag));
            if (err) {
            ...
            if (lfs_tag_type2(tag) == LFS_TYPE_CCRC) {
                // check the crc attr
                uint32_t dcrc;
                err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->block_size, dir->pair[0],
            ...
            err = lfs_bd_crc(lfs, NULL, &lfs->rcache, lfs->cfg->block_size, dir->pair[0],
                             off + sizeof(tag), lfs_tag_dsize(tag) - sizeof(tag), &crc);
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
                    break;
                }
            ...
            if (lfs_tag_type1(tag) == LFS_TYPE_NAME) {
                ...
                if (tag ==
                    (LFS_MKTAG(LFS_TYPE_DELETE, 0, 0) | (LFS_MKTAG(0, 0x3ff, 0) & tempbesttag))) {
                ...
                err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->block_size, dir->pair[0],
                                  off + sizeof(tag), &fcrc, sizeof(fcrc));
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        break;
                    }
        ...
        if (maybeerased && dir->off % lfs->cfg->prog_size == 0) {
            ...
            if (lfs_fs_disk_version(lfs) < 0x00020001) {
                ...
                int err = lfs_bd_crc(lfs, NULL, &lfs->rcache, lfs->cfg->block_size, dir->pair[0],
                                     dir->off, fcrc.size, &fcrc_);
                if (err && err != LFS_ERR_CORRUPT) {
                    return err;
                }
...
static lfs_stag_t lfs_dir_find(lfs_t *lfs, lfs_mdir_t *dir, const char **path, uint16_t *id) {
    ...
    nextname:
        ...
        *path = name;
        ...
        while (true) {
            tag = lfs_dir_fetchmatch(lfs, dir, dir->tail, LFS_MKTAG(0x780, 0, 0),
                                     LFS_MKTAG(LFS_TYPE_NAME, 0, namelen), id, lfs_dir_find_match,
...
#ifndef LFS_READONLY
static int
lfs_dir_commitprog(lfs_t *lfs, struct lfs_commit *commit, const void *buffer, lfs_size_t size) {
    int err = lfs_bd_prog(lfs, &lfs->pcache, &lfs->rcache, false, commit->block, commit->off,
...
#ifndef LFS_READONLY
static int
lfs_dir_commitattr(lfs_t *lfs, struct lfs_commit *commit, lfs_tag_t tag, const void *buffer) {
    // check if we fit
    ...
    if (!(tag & 0x80000000)) {
        ...
        for (lfs_off_t i = 0; i < dsize - sizeof(tag); i++) {
            ...
            uint8_t dat;
            err = lfs_bd_read(lfs, NULL, &lfs->rcache, dsize - sizeof(tag) - i, disk->block,
                              disk->off + i, &dat, 1);
            if (err) {
                return err;
            }
...
static int lfs_dir_commitcrc(lfs_t *lfs, struct lfs_commit *commit) {
    ...
    const lfs_off_t end = lfs_alignup(
        lfs_min(commit->off + 5 * sizeof(uint32_t), lfs->cfg->block_size), lfs->cfg->prog_size);
    ...
    while (commit->off < end) {
        ...
        if (noff >= end && noff <= lfs->cfg->block_size - lfs->cfg->prog_size) {
            ...
            int err = lfs_bd_read(lfs, NULL, &lfs->rcache, lfs->cfg->prog_size, commit->block, noff,
                                  &eperturb, 1);
            if (err && err != LFS_ERR_CORRUPT) {
                return err;
            }
    ...
    #endif
            {
                ...
                err = lfs_bd_crc(lfs, NULL, &lfs->rcache, lfs->cfg->prog_size, commit->block, noff,
                                 fcrc.size, &fcrc.crc);
                if (err && err != LFS_ERR_CORRUPT) {
                    return err;
                }
                ...
                err = lfs_dir_commitattr(
                    lfs, commit, LFS_MKTAG(LFS_TYPE_FCRC, 0x3ff, sizeof(struct lfs_fcrc)), &fcrc);
                if (err) {
                    return err;
                }
            }
        ...
        lfs_tag_t ntag = LFS_MKTAG(LFS_TYPE_CCRC + (((uint8_t)~eperturb) >> 7), 0x3ff,
    ...
    int       err = lfs_bd_crc(lfs, NULL, &lfs->rcache, off1 + sizeof(uint32_t), commit->block, off,
                               off1 - off, &crc);
    if (err) {
        return err;
    }
    ...
    err = lfs_bd_crc(lfs, NULL, &lfs->rcache, sizeof(uint32_t), commit->block, off1,
                     sizeof(uint32_t), &crc);
    if (err) {
        return err;
    }
...
static int lfs_dir_alloc(lfs_t *lfs, lfs_mdir_t *dir) {
    ...
    int err = lfs_bd_read(lfs, NULL, &lfs->rcache, sizeof(dir->rev), dir->pair[0], 0, &dir->rev,
                          sizeof(dir->rev));
    dir->rev = lfs_fromle32(dir->rev);
    if (err && err != LFS_ERR_CORRUPT) {
        return err;
...
static int lfs_dir_drop(lfs_t *lfs, lfs_mdir_t *dir, lfs_mdir_t *tail) {
    ...
    err = lfs_dir_commit(
        lfs, dir, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_TAIL + tail->split, 0x3ff, 8), tail->tail}));
    ...
    while (true) {
        {
            ...
            err = lfs_dir_traverse(lfs, source, 0, 0xffffffff, attrs, attrcount,
            ...
            if (!lfs_pair_isnull(dir->tail)) {
                ...
                err = lfs_dir_commitattr(
                    lfs, &commit, LFS_MKTAG(LFS_TYPE_TAIL + dir->split, 0x3ff, 8), dir->tail);
                lfs_pair_fromle32(dir->tail);
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        goto relocate;
            ...
            if (!lfs_gstate_iszero(&delta)) {
                ...
                err = lfs_dir_commitattr(
                    lfs, &commit, LFS_MKTAG(LFS_TYPE_MOVESTATE, 0x3ff, sizeof(delta)), &delta);
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        goto relocate;
                    }
    ...
    while (true) {
        ...
        while (end - split > 1) {
            ...
            int        err = lfs_dir_traverse(lfs, source, 0, 0xffffffff, attrs, attrcount,
                                              LFS_MKTAG(0x400, 0x3ff, 0), LFS_MKTAG(LFS_TYPE_NAME, 0, 0),
                                              split, end, -split, lfs_dir_commit_size, &size);
            if (err) {
                return err;
            }
            ...
            if (end - split < 0xff &&
                size <= lfs_min(metadata_max - 40,
                                lfs_alignup(metadata_max / 2, lfs->cfg->prog_size))) {
                break;
            }
...
static int lfs_dir_relocatingcommit(lfs_t                  *lfs,
    ...
    if (dir->erased && dir->count < 0xff) {
        ...
        lfs_pair_tole32(dir->tail);
        int err =
            lfs_dir_traverse(lfs, dir, dir->off, dir->etag, attrs, attrcount, 0, 0, 0, 0, 0,
            ...
            if (d->m.pair != pair) {
                for (int i = 0; i < attrcount; i++) {
                    if (lfs_tag_type3(attrs[i].tag) == LFS_TYPE_DELETE &&
                        d->id == lfs_tag_id(attrs[i].tag) && d->type != LFS_TYPE_DIR) {
...
#ifndef LFS_READONLY
static int
lfs_dir_orphaningcommit(lfs_t *lfs, lfs_mdir_t *dir, const struct lfs_mattr *attrs, int attrcount) {
    ...
    if (state == LFS_OK_DROPPED) {
        ...
        state = lfs_dir_relocatingcommit(
            lfs, &pdir, lpair,
            LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_TAIL + dir->split, 0x3ff, 8), dir->tail}), NULL);
        lfs_pair_fromle32(dir->tail);
        if (state < 0) {
            return state;
        }
...
static int lfs_mkdir_(lfs_t *lfs, const char *path) {
    ...
    err =
        lfs_dir_commit(lfs, &dir, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 8), pred.tail}));
...
static int lfs_ctz_find(lfs_t             *lfs,
    ...
    while (current > target) {
        ...
        int err =
            lfs_bd_read(lfs, pcache, rcache, sizeof(head), head, 4 * skip, &head, sizeof(head));
    ...
    while (true) {
...

        {
            ...
            for (lfs_off_t i = 0; i < skips; i++) {
                ...
                if (i != skips - 1) {
                    err = lfs_bd_read(lfs, NULL, rcache, sizeof(nhead), nhead, 4 * i, &nhead,
                                      sizeof(nhead));
                    nhead = lfs_fromle32(nhead);
                    if (err) {
                        return err;
...
static int lfs_ctz_traverse(lfs_t             *lfs,
    ...
    while (true) {
        ...
        lfs_block_t heads[2];
        int         count = 2 - (index & 1);
        err = lfs_bd_read(lfs, pcache, rcache, count * sizeof(head), head, 0, &heads,
                          count * sizeof(head));
...
static int lfs_file_opencfg_(lfs_t                        *lfs,
    ...
    if (tag == LFS_ERR_NOENT) {
        ...
        // it may happen that the file name doesn't fit in the metadata blocks, e.g., a 256 byte
        // file name will not fit in a 128 byte block.
    ...
    for (unsigned i = 0; i < file->cfg->attr_count; i++) {
        ...
        if ((file->flags & LFS_O_RDONLY) == LFS_O_RDONLY) {
            lfs_stag_t res = lfs_dir_get(lfs, &file->m, LFS_MKTAG(0x7ff, 0x3ff, 0),
    ...
    while (true) {
        ...
        for (lfs_off_t i = 0; i < file->off; i++) {
            ...
            if (file->flags & LFS_F_INLINE) {
                ...
                err = lfs_bd_read(lfs, &file->cache, &lfs->rcache, file->off - i, file->block, i,
                                  &data, 1);
                if (err) {
                    return err;
                }
...
lfs_file_flushedread(lfs_t *lfs, lfs_file_t *file, void *buffer, lfs_size_t size) {
    ...
    while (nsize > 0) {
        ...
        if (!(file->flags & LFS_F_READING) || file->off == lfs->cfg->block_size) {
            if (!(file->flags & LFS_F_INLINE)) {
                int err = lfs_ctz_find(lfs, NULL, &file->cache, file->ctz.head, file->ctz.size,
        ...
        if (file->flags & LFS_F_INLINE) {
            int err = lfs_dir_getread(
                lfs, &file->m, NULL, &file->cache, lfs->cfg->block_size, LFS_MKTAG(0xfff, 0x1ff, 0),
                LFS_MKTAG(LFS_TYPE_INLINESTRUCT, file->id, 0), file->off, data, diff);
            if (err) {
                return err;
            }
        } else {
            int err = lfs_bd_read(lfs, NULL, &file->cache, lfs->cfg->block_size, file->block,
                                  file->off, data, diff);
            if (err) {
                return err;
            }
    ...
    while (nsize > 0) {
        ...
        if (!(file->flags & LFS_F_WRITING) || file->off == lfs->cfg->block_size) {
            if (!(file->flags & LFS_F_INLINE)) {
                ...
                lfs_alloc_ckpoint(lfs);
                int err = lfs_ctz_extend(lfs, &file->cache, &lfs->rcache, file->block, file->pos,
                                         &file->block, &file->off);
                if (err) {
                    file->flags |= LFS_F_ERRED;
                    return err;
        ...
        lfs_size_t diff = lfs_min(nsize, lfs->cfg->block_size - file->off);
        while (true) {
            int err = lfs_bd_prog(lfs, &file->cache, &lfs->rcache, true, file->block, file->off,
                                  data, diff);
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
                    goto relocate;
...
static int lfs_file_truncate_(lfs_t *lfs, lfs_file_t *file, lfs_off_t size) {
    ...
    if (size < oldsize) {
        ...
        if (size <= lfs->inline_max) {
            ...
            err = lfs_ctz_find(lfs, NULL, &file->cache, file->ctz.head, file->ctz.size, size - 1,
                               &file->block, &(lfs_off_t){0});
            if (err) {
                return err;
            }
...
static int lfs_rename_(lfs_t *lfs, const char *oldpath, const char *newpath) {
    ...
    if (!samepair && lfs_gstate_hasmove(&lfs->gstate)) {
        ...
        lfs_fs_prepmove(lfs, 0x3ff, NULL);
        err = lfs_dir_commit(
            lfs, &oldcwd, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_DELETE, lfs_tag_id(oldtag), 0), NULL}));
        if (err) {
            lfs->mlist = prevdir.next;
            return err;
        }
    ...
    lfs->mlist = prevdir.next;
    if (lfs_gstate_hasorphans(&lfs->gstate)) {
        LFS_ASSERT(prevtag != LFS_ERR_NOENT && lfs_tag_type3(prevtag) == LFS_TYPE_DIR);
...
static lfs_ssize_t
lfs_getattr_(lfs_t *lfs, const char *path, uint8_t type, void *buffer, lfs_size_t size) {
    lfs_mdir_t cwd;
    lfs_stag_t tag = lfs_dir_find(lfs, &cwd, &path, NULL);
    if (tag < 0) {
        return tag;
    ...
    tag =
        lfs_dir_get(lfs, &cwd, LFS_MKTAG(0x7ff, 0x3ff, 0),
...
#ifndef LFS_READONLY
static int
lfs_commitattr(lfs_t *lfs, const char *path, uint8_t type, const void *buffer, lfs_size_t size) {
    lfs_mdir_t cwd;
    lfs_stag_t tag = lfs_dir_find(lfs, &cwd, &path, NULL);
    if (tag < 0) {
        return tag;
...
#ifndef LFS_READONLY
static int
lfs_setattr_(lfs_t *lfs, const char *path, uint8_t type, const void *buffer, lfs_size_t size) {
...
static int lfs_init(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    LFS_ASSERT(!lfs->cfg->disk_version ||
               ((0xffff & (lfs->cfg->disk_version >> 16)) == LFS_DISK_VERSION_MAJOR &&
    ...
    LFS_ASSERT(lfs->cfg->inline_max == (lfs_size_t)-1 || lfs->cfg->inline_max <= lfs->attr_max);
    LFS_ASSERT(lfs->cfg->inline_max == (lfs_size_t)-1 ||
               lfs->cfg->inline_max <=
    ...
    if (lfs->inline_max == (lfs_size_t)-1) {
        ...
        lfs->inline_max =
            lfs_min(lfs->cfg->cache_size,
                    lfs_min(lfs->attr_max, ((lfs->cfg->metadata_max) ? lfs->cfg->metadata_max
                                                                     : lfs->cfg->block_size) /
                                               8));
    }
...
static int lfs_mount_(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        lfs_stag_t tag = lfs_dir_fetchmatch(
            lfs, &dir, dir.tail, LFS_MKTAG(0x7ff, 0x3ff, 0), LFS_MKTAG(LFS_TYPE_SUPERBLOCK, 0, 8),
            NULL, lfs_dir_find_match, &(struct lfs_dir_find_match){lfs, "littlefs", 8});
        if (tag < 0) {
            err = tag;
            goto cleanup;
        }
        ...
        if (tag && !lfs_tag_isdelete(tag)) {
            ...
            if (major_version != lfs_fs_disk_version_major(lfs) ||
                ...
                LFS_ERROR("Invalid version "
                          ...
                          major_version, minor_version, lfs_fs_disk_version_major(lfs),
            ...
            if (superblock.block_size != lfs->cfg->block_size) {
                LFS_ERROR("Invalid block size (%" PRIu32 " != %" PRIu32 ")", superblock.block_size,
                          lfs->cfg->block_size);
                err = LFS_ERR_INVAL;
                goto cleanup;
            }
...
static int lfs_unmount_(lfs_t *lfs) { return lfs_deinit(lfs); }
...
static int lfs_fs_stat_(lfs_t *lfs, struct lfs_fsinfo *fsinfo) {
    ...
    if (!lfs_gstate_needssuperblock(&lfs->gstate)) {
        ...
        lfs_superblock_t superblock;
        lfs_stag_t       tag =
            lfs_dir_get(lfs, &dir, LFS_MKTAG(0x7ff, 0x3ff, 0),
                        LFS_MKTAG(LFS_TYPE_INLINESTRUCT, 0, sizeof(superblock)), &superblock);
        if (tag < 0) {
            return tag;
        }
...
int lfs_fs_traverse_(lfs_t *lfs,
    ...
    for (lfs_file_t *f = (lfs_file_t *)lfs->mlist; f; f = f->next) {
        ...
        if ((f->flags & LFS_F_DIRTY) && !(f->flags & LFS_F_INLINE)) {
            int err =
                lfs_ctz_traverse(lfs, &f->cache, &lfs->rcache, f->ctz.head, f->ctz.size, cb, data);
            if (err) {
                return err;
            }
        }
...
static int lfs_fs_parent_match(void *data, lfs_tag_t tag, const void *buffer) {
    ...
    lfs_block_t child[2];
    int err = lfs_bd_read(lfs, &lfs->pcache, &lfs->rcache, lfs->cfg->block_size, disk->block,
                          disk->off, &child, sizeof(child));
    if (err) {
        return err;
    }
...
static lfs_stag_t lfs_fs_parent(lfs_t *lfs, const lfs_block_t pair[2], lfs_mdir_t *parent) {
    ...
    while (!lfs_pair_isnull(parent->tail)) {
        ...
        lfs_stag_t tag =
            lfs_dir_fetchmatch(lfs, parent, parent->tail, LFS_MKTAG(0x7ff, 0, 0x3ff),
...
static int lfs_fs_desuperblock(lfs_t *lfs) {
    ...
    err = lfs_dir_commit(
        lfs, &root,
        LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_INLINESTRUCT, 0, sizeof(superblock)), &superblock}));
    if (err) {
        return err;
    }
        ...
        while (!lfs_pair_isnull(pdir.tail)) {
            ...
            if (!pdir.split) {
                ...
                if (pass == 0 && tag != LFS_ERR_NOENT) {
                    lfs_block_t pair[2];
                    lfs_stag_t  state =
                        lfs_dir_get(lfs, &parent, LFS_MKTAG(0x7ff, 0x3ff, 0), tag, pair);
                    if (state < 0) {
                        return state;
                    }
                    lfs_pair_fromle32(pair);
                ...
                if (pass == 1 && tag == LFS_ERR_NOENT && powerloss) {
                    // we are an orphan
                    LFS_DEBUG("Fixing orphan {0x%" PRIx32 ", 0x%" PRIx32 "}", pdir.tail[0],
                              pdir.tail[1]);
...
static int lfs_fs_grow_(lfs_t *lfs, lfs_size_t block_count) {
    ...
    lfs_superblock_t superblock;
    lfs_stag_t       tag =
        lfs_dir_get(lfs, &root, LFS_MKTAG(0x7ff, 0x3ff, 0),
                    LFS_MKTAG(LFS_TYPE_INLINESTRUCT, 0, sizeof(superblock)), &superblock);
    if (tag < 0) {
        return tag;
    }
...
static int
lfs1_bd_read(lfs_t *lfs, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size) {
...
static int
lfs1_bd_crc(lfs_t *lfs, lfs_block_t block, lfs_off_t off, lfs_size_t size, uint32_t *crc) {
...
static int lfs1_dir_fetch(lfs_t *lfs, lfs1_dir_t *dir, const lfs_block_t pair[2]) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        lfs1_dir_fromle32(&test);
        err =
            lfs1_bd_crc(lfs, tpair[i], sizeof(test), (0x7fffffff & test.size) - sizeof(test), &crc);
        if (err) {
            if (err == LFS_ERR_CORRUPT) {
                continue;
            }
    ...
    while (true) {
        ...
        while (dir.off + sizeof(entry.d) <= (0x7fffffff & dir.d.size) - 4) {
            ...
            if ((0x70 & entry.d.type) == (0x70 & LFS1_TYPE_REG)) {
                err = lfs_ctz_traverse(lfs, NULL, &lfs->rcache, entry.d.u.file.head,
...
static int lfs1_mount(lfs_t *lfs, struct lfs1 *lfs1, const struct lfs_config *cfg) {
    int err = 0;
    {
        ...
        if (!err) {
            err =
                lfs1_bd_read(lfs, dir.pair[0], sizeof(dir.d), &superblock.d, sizeof(superblock.d));
...
static int lfs1_unmount(lfs_t *lfs) { return lfs_deinit(lfs); }
...

    {
        ...
        while (!lfs_pair_isnull(dir1.d.tail)) {
            ...
            while (true) {
                ...
                err =
                    lfs1_bd_read(lfs, dir1.pair[0], entry1.off + 4 + entry1.d.elen + entry1.d.alen,
                                 name, entry1.d.nlen);
                if (err) {
                    goto cleanup;
                }
                ...
                err = lfs_dir_commit(
                    ...
                    LFS_MKATTRS(
                        ...
                        {LFS_MKTAG_IF_ELSE(isdir, LFS_TYPE_DIR, id, entry1.d.nlen, LFS_TYPE_REG, id,
                                           entry1.d.nlen),
                         name},
                        {LFS_MKTAG_IF_ELSE(isdir, LFS_TYPE_DIRSTRUCT, id, sizeof(entry1.d.u),
            ...
            if (!lfs_pair_isnull(dir1.d.tail)) {
                ...
                err = lfs_dir_commit(
                    lfs, &dir2, LFS_MKATTRS({LFS_MKTAG(LFS_TYPE_SOFTTAIL, 0x3ff, 8), dir1.d.tail}));
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    LFS_TRACE("lfs_format(%p, %p {.context=%p, "
              ...
              (void *)(uintptr_t)cfg->sync, cfg->read_size, cfg->prog_size, cfg->block_size,
              cfg->block_count, cfg->block_cycles, cfg->cache_size, cfg->lookahead_size,
              cfg->read_buffer, cfg->prog_buffer, cfg->lookahead_buffer, cfg->name_max,
...
lfs_ssize_t lfs_getattr(lfs_t *lfs, const char *path, uint8_t type, void *buffer, lfs_size_t size) {
    ...
    LFS_TRACE("lfs_getattr(%p, \"%s\", %" PRIu8 ", %p, %" PRIu32 ")", (void *)lfs, path, type,
              buffer, size);
...
int lfs_setattr(lfs_t *lfs, const char *path, uint8_t type, const void *buffer, lfs_size_t size) {
    ...
    LFS_TRACE("lfs_setattr(%p, \"%s\", %" PRIu8 ", %p, %" PRIu32 ")", (void *)lfs, path, type,
              buffer, size);
...
int lfs_file_open(lfs_t *lfs, lfs_file_t *file, const char *path, int flags) {
    ...
    LFS_TRACE("lfs_file_open(%p, %p, \"%s\", %x)", (void *)lfs, (void *)file, path,
              (unsigned)flags);
...
int lfs_file_opencfg(lfs_t                        *lfs,
    ...
    LFS_TRACE("lfs_file_opencfg(%p, %p, \"%s\", %x, %p {"
              ...
              (void *)lfs, (void *)file, path, (unsigned)flags, (void *)cfg, cfg->buffer,
