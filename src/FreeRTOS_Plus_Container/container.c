...
static int get_elf_by_name(ELF_WRAP *wrap, const char *name) {
    ...
    if (pxFS == NULL || pxFS->fs_ops == NULL) {
        uart_puts("File system not initialized\r\n");
        return pdFAIL;
    }
...
#define MY_DEBUG
    ...
    // 输出/根目录所有文件信息（ls）
    {
        ...
        uart_puts("Listing root directory:\r\n");
        ...
        while (1) {
            ...
            if (res < 0) {
                uart_puts("Failed to read directory entry\r\n");
            ...
            uart_puts((info.type == LFS_TYPE_DIR) ? "DIR " : "FILE ");
            uart_puts(info.name);
            uart_puts("\r\n");
        }
    ...
    if (ret < 0) {
        uart_puts("Failed to open ELF file: ");
        uart_puts(file_path);
        uart_puts("\r\n");
        return pdFAIL;
    }
    ...
    if (file_size <= 0) {
        uart_puts("Invalid ELF file size\r\n");
    ...
    if (elf_buffer == NULL) {
        uart_puts("Failed to allocate memory for ELF\r\n");
        lfs_ops->file_close(&file);
        return pdFAIL;
    }
    ...
    if (ret != file_size) {
        uart_puts("Failed to read ELF file\r\n");
    ...
    uart_puts("Loaded ELF from: ");
    uart_puts(file_path);
    uart_puts(" (");
    uart_puthex((uint64_t)file_size);
    uart_puts(" bytes)\r\n");
...
/* Verify that all isolation mechanisms are in place before running user code */
    ...
    if (xTaskChroot(pxContainer->pcRootPath) != pdPASS) {
        pxContainer->eState = CONTAINER_STATE_ERROR;
#ifdef MY_DEBUG
        uart_puts("ERROR: Failed to chroot to ");
        uart_puts(pxContainer->pcRootPath);
        uart_puts("\r\n");
#endif
        vTaskDelete(NULL);
        return;
    ...
    if (xSemaphoreTake(xContainerMutex, portMAX_DELAY) == pdTRUE) {
        ...
        if (pxContainer != NULL && pxContainer->eState == CONTAINER_STATE_STOPPED) {
            ...
            if (pxTaskParams == NULL) {
                xSemaphoreGive(xContainerMutex);
                uart_puts("ERROR: Failed to allocate task parameters.\r\n");
                return pdFAIL;
            }
            ...
            if (pxContainer->xReadySemaphore == NULL) {
                ...
                xSemaphoreGive(xContainerMutex);
                uart_puts("ERROR: Failed to create ready semaphore.\r\n");
                return pdFAIL;
            }
...
/* 1. Add task to CGroup FIRST - following cgroup_example pattern */
                ...
                if (pxContainer->xCGroup != NULL && pxContainer->xTaskHandle != NULL) {
                    ...
                    if (xCGroupResult != pdPASS) {
                        ...
                        vPortFree(pxTaskParams);
                        xSemaphoreGive(xContainerMutex);
                        uart_puts("ERROR: Failed to add task to CGroup.\r\n");
                        return pdFAIL;
                    }
                }
#endif
                ...
                uart_puts("ERROR: Failed to create container task.\r\n");
                if (pxContainer->xReadySemaphore != NULL) {
                    vSemaphoreDelete(pxContainer->xReadySemaphore);
                    pxContainer->xReadySemaphore = NULL;
                }
        ...
        uart_puts("ERROR: Failed to acquire container mutex.\r\n");
    }

    uart_puts("Container start result: ");
    uart_puthex(xResult);
    uart_puts("\r\n");
    return xResult;
}
...
static const CLI_Command_Definition_t xLsCmd = {
    ...
    xLsCommand, -1 /* Variable number of parameters (0 or 1) */
};
#endif
