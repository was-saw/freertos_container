...
BaseType_t xContainerUnpackImage(const char *pcImagePath, uint32_t ulContainerID)
{
...
#ifdef MY_DEBUG
        uart_puts("Failed to ensure /var exists\r\n");
#endif
        return pdFAIL;
    }
...
#ifdef MY_DEBUG
        uart_puts("Failed to ensure /var/container exists\r\n");
#endif
        return pdFAIL;
    }
...
#ifdef MY_DEBUG
        uart_puts("Container directory already exists");
#endif
    ...
    if (result != LFS_ERR_OK) {
#ifdef MY_DEBUG
        uart_puts("Failed to create container directory\r\n");
#endif
        return pdFAIL;
    }
    ...
    if (result < 0) {
#ifdef MY_DEBUG
        uart_puts("Failed to open image file\r\n");
#endif
        goto cleanup_dir;
    }
    ...
    if (result != 1) {
#ifdef MY_DEBUG
        uart_puts("Failed to read number of files\r\n");
#endif
        goto cleanup_file;
    }
    ...
    for (i = 0; i < ucNumFiles; i++) {
        ...
        if (result != 8) {
#ifdef MY_DEBUG
            uart_puts("Failed to read file size\r\n");
#endif
            goto cleanup_file;
        }
        ...
        if (result != 256) {
#ifdef MY_DEBUG
            uart_puts("Failed to read filename\r\n");
#endif
            goto cleanup_file;
        }
...
BaseType_t xContainerPackImage(uint32_t ulContainerID, const char *pcImagePath)
{
    ...
    while (ucFileCount < ucNumFiles) {
        ...
        my_memset(aucFilenameBuf, 0, 256);
